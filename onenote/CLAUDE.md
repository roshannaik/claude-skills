...
python3 scripts/sync.py sync --force         # bypass --max-changes safety threshold
...
| `onenote_chunks.py` | **Adaptive mechanical chunker.** Walks top-level HTML blocks in document order; paragraph-packs short paragraphs, treats multi-sentence paragraphs as own chunks, row-atomic chunks per table row with header re-prefix, balanced-tag scanning for nested tables. **Lists** are captured by balanced-tag scan (not non-greedy regex — that truncated nested same-tag lists) and rendered via `html_to_md` (DOM) so nesting, numbering, and the context line(s) sitting directly under a `<ul>`/`<ol>` survive; small lists (≤`CHUNK_HARD_MAX_CHARS`) pack inline with neighbours, oversized lists split at **top-level item boundaries** with a capped (`LIST_LEAD_CTX_CAP`) lead-context re-prefix on every chunk — the list analogue of the table header re-prefix, so a sub-bullet never loses its parent context. Emits typed `Chunk` objects: `text`, `summary`, `image`, `image_ocr`, `image_caption`, `pdf`, `audio`, `video_transcript`, `audio_transcript`. Text chunks carry `embed_text`; media chunks carry `resource_id`/`mime`/`filename`. |
| `onenote_embeddings.py` | Gemini embeddings build + query, unified text+media vector space (`gemini-embedding-2-preview` @ 768d). Stores `cache/embeddings.npz` (float32, L2-normalized, with `kinds` array) + `cache/embeddings_meta.json`. **Build is concurrent**: `MAX_CONCURRENT` (=8) workers call the API in parallel via `ThreadPoolExecutor`; throughput self-tunes via the `retryDelay` hint from 429s (no fixed inter-call sleep). Workers mutate `new_vecs / new_kinds / new_chunks_meta` under `store_lock`; `_merge_and_save` snapshots them under the same lock to avoid torn checkpoints. Query path: single matmul; subject-aware filtering; top-K pages × max-N chunks/page. Public API: `semantic_search`, `query_by_page` (uses an existing page's summary vector — skips Gemini call), `get_chunk_text` (re-chunks host page to recover `embed_text`). **`force` is scope-aware**: a full-corpus force wipes and rebuilds everything, but a force scoped to a page subset (`page_ids`/`pages_file`) re-embeds only those pages and **merges** into the existing store (`wipe = force and not subset`) — other notebooks are preserved, enabling staged per-notebook rebuilds. |
| `onenote_genai.py` | Shared Gemini client + `with_retry` helper. On a 429/503/`RESOURCE_EXHAUSTED`, parses the `retryDelay` field from the error body (`err.details['error']['details'][n]['retryDelay']`) and waits exactly that long (+0.3s buffer); falls back to exponential backoff if the hint is absent. All LLM call sites go through it. |
...
| `cache_backup.py` | Backup (`backup [--force]`), restore (`restore [--yes] [--force]`), and undo (`undo [--yes]`) the entire `cache/` to/from OneDrive root (`onenote_cache.tar.gz`) via Graph Drive API. Both backup and restore consult a `{full_ok_ms, partial_ms}` version pair stored in (a) `cache/.cache_version.json` (written by sync.py on cache-mutating runs) and (b) the OneDrive DriveItem's `description` field (stamped via post-upload PATCH; personal OneDrive rejects `description` in upload-session metadata, and HTML-encodes `:` in description on read so cache_backup `html.unescape`s before parsing). The comparison rule engine (see module docstring) returns one of skip / ok / warn / unknown and the appropriate human suggestion ("restore first" / "run a clean local sync first" / etc.); `--force` overrides skip+warn verdicts. Restore prints current-vs-remote stats, applies the mirror rules, prompts (unless `--yes`), then snapshots the live cache via a fast rename to a sibling `cache.previous/` (single rolling snapshot) before extracting the new archive. `undo` consumes that snapshot. Uses the same auth as the rest of the skill. |
| `sync.py` | Single-shot cache sync orchestrator. Detects dirty notebooks via `last_modified`, refreshes them, prunes orphans, triggers incremental embedding rebuild. fcntl-locked + self-kill via SIGALRM after `--max-duration`; JSONL log at `cache/sync.log`. `--max-changes N` (default 40) aborts the run if either the fetch step or the embed step would touch more than N pages — runaway-rebuild guard against Graph `lastModifiedDateTime` flutter regressions; `--force` bypasses. Designed to be safe to fire from cron / launchd / keystroke. |
...
- **Chunker logic change is invisible to incremental rebuild** — carry-forward keys off `last_modified` + chunk-id presence, not a chunker version, so editing `onenote_chunks.py` does NOT auto-trigger a re-embed. After such a change, re-embed affected pages explicitly: `build_embeddings.py --force` (whole corpus, wipes + rebuilds) or `--force --pages-file <subset>` (scoped — re-embeds only those pages, merges into the store, preserves other notebooks). Until then, `get_chunk_text` / `query --with-text` may fail to recover old list/text chunks whose IDs no longer match the new chunker output.
...
- **Cache version invariant**: `cache/.cache_version.json` carries `{full_ok_ms, partial_ms}` with `partial_ms >= full_ok_ms` at all times. `sync.py` advances `partial_ms` on any cache-mutating run (`pages_added/modified/deleted ≥ 1` OR `embeddings.pages_rebuilt ≥ 1` — explicitly NOT `pages_fetched`, which flutters per Graph's broken `lastModifiedDateTime` for unchanged content) and advances `full_ok_ms` only when the run was fully clean (`status=ok` AND zero refresh/fetch errors). The file is written by `_update_cache_version` in sync.py; if you add new cache-write paths outside sync.py that should be reflected in backups, call into the same helper. `cache_backup.py`'s rule engine reads this file plus the remote DriveItem's `description` JSON to decide skip/ok/warn (see module docstring there).

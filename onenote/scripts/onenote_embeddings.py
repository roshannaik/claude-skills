#!python3
"""Chunked, multimodal, unified vector space semantic search.
...
Storage:
    cache/embeddings.npz         ids, vectors, kinds
    cache/embeddings_meta.json   model, dim, pages, chunks
...
def build_embeddings(page_ids: list = None, pages_file: str = None,
    ...
    if meta.get('model') != MODEL or meta.get('dim') != EMBED_DIM:
        print(f'  model/dim changed ({meta.get("model")}, {meta.get("dim")}) → '
              f'({MODEL}, {EMBED_DIM}); wiping state', file=sys.stderr)
...
def _load_for_query():
    ...
    global _query_state
    if not EMBEDDINGS_NPZ.exists():
        raise SystemExit('No embeddings found. Run: build_embeddings.py')
